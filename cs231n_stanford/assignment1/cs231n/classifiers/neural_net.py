...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    Z = np.dot(X, W1) + b1 # fully connected process (N,H)
    A = np.maximum(0, Z) # Relu function (N, H)
    scores = np.dot(A, W2) + b2 # fully connected process (N,C) 
    scores -= scores.max()
    ...
    loss = -np.sum(np.log(loss))/N + reg * np.sum(W1*W1) + reg * np.sum(W2*W2)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
